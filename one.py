...
def draw(draw_info,algo_name,ascending):
    ...
    title=draw_info.FONT.render(f"{algo_name}-{'Ascending' if ascending else 'Decending'}",1,draw_info.BLUE)
    draw_info.window.blit(title,((draw_info.width/2)-(title.get_width()/2),5))

    controls=draw_info.FONT.render("R-RESET | SPACE-Start Sorting | A-Asce | D-Des ",1,draw_info.BLACK)
...
def draw_list(draw_info,color_positions={},clear_background=False):
    ...
    for i,val in enumerate(lst):
        ...
        color=draw_info.GRADIENTS[i%3] #0,1,2
